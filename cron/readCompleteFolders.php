...
if (count($folders) > 0) {
    foreach ($folders as $folder) {
        ...
        $message = null;
        $filecount = 0;
        if (is_dir(PICTURE_FOLDER . $folder)) {//Tätä testattava testissä Seuraavassa ajossa
            if (strpos(".", $folder) !== false) {
                $object_name = str_lreplace("_", ".", $folder);
                $object_name = str_replace("_", ":", $object_name);
                $data = array("isCollectionReady" => $object_name);
                $tmp = callRest("POST", "/rest/collections.php", $data, true); //Is collection folder ready...
            } else {
            ...
            if (isset($tmp->valmis) && $tmp->valmis == 1 && $tmp->tarkistus == 0) {
                if ($tmp->objektin_id == "") {//Yhteys on välillä todella pitkä ja tästä syystä ei saada aina noita ID:tä... Sen takia se on osa tätä prosessia...
                    ...
                    $object_id = callRest("POST", "/rest/getObjectId.php", array("search" => $object_name, "re_tries" => RE_TRIES), true);
                    if ($object_id != "" && is_numeric($object_id)) { //Let´s save also object id
                        ...
                        $tmp2 = callRest("POST", "/rest/editCollection.php", $data2);
                        ...
                        $tmp3 = callRest("POST", "/rest/changeProsessingStatus.php", $data1, true);
                ...
                if (is_numeric($object_id) && is_numeric($tmp->tarkistus) && $tmp->tarkistus == 0) { //Pitää löytyi tietokannasta
                    ...
                    $tmp_excludes = array('..', '.');
                    $tmp_array = array_merge($tmp_excludes, $poistettavat_tiedostot);
                    $tmp_files = array_diff(scandir(PICTURE_FOLDER . $folder), $tmp_array);
                    if ($tmp_files) {
                        $added = 0;
                        $filecount = count($tmp_files);
                        foreach ($poistettavat_tiedostot as $tiedosto) {
                            if (file_exists(PICTURE_FOLDER . $folder . $tiedosto)) {
                                unlink(PICTURE_FOLDER . $folder . $tiedosto);
                            }
                        }

                        foreach ($tmp_files as $file) {
                            $tmp_data = array("newFile" => 1, "file" => $file, "folder" => PICTURE_FOLDER . $folder, "row_id" => $tmp->rivi_id);
                            $ok = callRest("POST", "/rest/addNewFile.php", $tmp_data, true);
                    ...
                    if ($filecount == 0) { //hakemisto on vielä tyhjä
                        ...
                        callRest("POST", "/rest/changeProsessingStatus.php", $data1, true);
                        continue;
                    }
                    ...
                    if ($filecount > 0 && $tmp3 > 0 && $added == $filecount) {
                        ...
                        callRest("POST", "/rest/editCollection.php", $data3);
                        ...
                        $removed = callRest("POST", "/rest/files.php", $tmp_data, true);
