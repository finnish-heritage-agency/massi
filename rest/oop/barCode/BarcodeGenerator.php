...
abstract class BarcodeGenerator {
    ...
    protected function getBarcodeData($code, $type) {
        switch (strtoupper($type)) {
            case $type == "TYPE_CODE_39": { // CODE 39 - ANSI MH10.8M-1983 - USD-3 - 3 of 9.
                    $arrcode = $this->barcode_code39($code, false, false);
                    break;
                }
            case $type == "TYPE_CODE_39_CHECKSUM": { // CODE 39 with checksum
                    $arrcode = $this->barcode_code39($code, false, true);
                    break;
                }
            case $type == "TYPE_CODE_39E": { // CODE 39 EXTENDED
                    $arrcode = $this->barcode_code39($code, true, false);
                    break;
                }
            case $type == "TYPE_CODE_39E_CHECKSUM": { // CODE 39 EXTENDED + CHECKSUM
                    $arrcode = $this->barcode_code39($code, true, true);
                    break;
                }
            case $type == "TYPE_CODE_93": { // CODE 93 - USS-93
                    $arrcode = $this->barcode_code93($code);
                    break;
                }
            case $type == "TYPE_STANDARD_2_5": { // Standard 2 of 5
                    $arrcode = $this->barcode_s25($code, false);
                    break;
                }
            case $type == "TYPE_STANDARD_2_5_CHECKSUM": { // Standard 2 of 5 + CHECKSUM
                    $arrcode = $this->barcode_s25($code, true);
                    break;
                }
            case $type == "TYPE_INTERLEAVED_2_5": { // Interleaved 2 of 5
                    $arrcode = $this->barcode_i25($code, false);
                    break;
                }
            case $type == "TYPE_INTERLEAVED_2_5_CHECKSUM": { // Interleaved 2 of 5 + CHECKSUM
                    $arrcode = $this->barcode_i25($code, true);
                    break;
                }
            case $type == "TYPE_CODE_128" : { // CODE 128
                    $arrcode = $this->barcode_c128($code, '');
                    break;
                }
            case $type == "TYPE_CODE_128_A": { // CODE 128 A
                    $arrcode = $this->barcode_c128($code, 'A');
                    break;
                }
            case $type == "TYPE_CODE_128_B": { // CODE 128 B
                    $arrcode = $this->barcode_c128($code, 'B');
                    break;
                }
            case $type == "TYPE_CODE_128_C": { // CODE 128 C
                    $arrcode = $this->barcode_c128($code, 'C');
                    break;
                }
            case $type == "TYPE_EAN_2": { // 2-Digits UPC-Based Extention
                    $arrcode = $this->barcode_eanext($code, 2);
                    break;
                }
            case $type == "TYPE_EAN_5": { // 5-Digits UPC-Based Extention
                    $arrcode = $this->barcode_eanext($code, 5);
                    break;
                }
            case $type == "TYPE_EAN_8": { // EAN 8
                    $arrcode = $this->barcode_eanupc($code, 8);
                    break;
                }
            case $type == "TYPE_EAN_13": { // EAN 13
                    $arrcode = $this->barcode_eanupc($code, 13);
                    break;
                }
            case $type == "TYPE_UPC_A": { // UPC-A
                    $arrcode = $this->barcode_eanupc($code, 12);
                    break;
                }
            case $type == "TYPE_UPC_E": { // UPC-E
                    $arrcode = $this->barcode_eanupc($code, 6);
                    break;
                }
            case $type == "TYPE_MSI": { // MSI (Variation of Plessey code)
                    $arrcode = $this->barcode_msi($code, false);
                    break;
                }
            case $type == "TYPE_MSI_CHECKSUM": { // MSI + CHECKSUM (modulo 11)
                    $arrcode = $this->barcode_msi($code, true);
                    break;
                }
            case $type == "TYPE_POSTNET": { // POSTNET
                    $arrcode = $this->barcode_postnet($code, false);
                    break;
                }
            case $type == "TYPE_PLANET": { // PLANET
                    $arrcode = $this->barcode_postnet($code, true);
                    break;
                }
            case $type == "TYPE_RMS4CC": { // RMS4CC (Royal Mail 4-state Customer Code) - CBC (Customer Bar Code)
                    $arrcode = $this->barcode_rms4cc($code, false);
                    break;
                }
            case $type == "TYPE_KIX": { // KIX (Klant index - Customer index)
                    $arrcode = $this->barcode_rms4cc($code, true);
                    break;
                }
            case $type == "TYPE_IMB": { // IMB - Intelligent Mail Barcode - Onecode - USPS-B-3200
                    $arrcode = $this->barcode_imb($code);
                    break;
                }
            case $type == "TYPE_CODABAR": { // CODABAR
                    $arrcode = $this->barcode_codabar($code);
                    break;
                }
            case $type == "TYPE_CODE_11": { // CODE 11
                    $arrcode = $this->barcode_code11($code);
                    break;
                }
            case $type == "TYPE_PHARMA_CODE": { // PHARMACODE
                    $arrcode = $this->barcode_pharmacode($code);
                    break;
                }
            case $type == "TYPE_PHARMA_CODE_TWO_TRACKS": { // PHARMACODE TWO-TRACKS
